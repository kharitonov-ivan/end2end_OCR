...
from model.modules.ASRN.fracPickup import fracPickup
...
class Attention(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes, num_embeddings=128, CUDA=True, batchSize = 20):
    ...
    # targets is nT * nB
    def forward(self, feats, text_length_, text_, test=False):

        text = torch.LongTensor(self.batchSize * 5)
        text_rev = torch.LongTensor(self.batchSize * 5)
        length = torch.IntTensor(self.batchSize)
        ...
        loadData(text, text_)
        loadData(length,text_length_)


        print(text_length)
        text_length = Variable()
        loadData(text_length)
...
class ASRN(nn.Module):
    ...
    def forward(self, input, length, text, text_rev=None, test=False):
        # conv features
        conv = self.cnn(input)
