...
parser.add_argument('--train_coco', required=True, help='path to dataset')  # !!!
...
parser.add_argument('--saveInterval', type=int, default=10000, help='Interval to be saved')
...
if opt.cuda:
    ...
    print('Launching MORAN without CUDA GPU')
...
if opt.cuda:
    print('Setting image, text and criterion to CUDA')
    MORAN.cuda()
...
#                 print(torch.mean(preds0_prob[text_begin:text_begin+len(sim_preds0[j].split('$')[0]+'$')]))
...
#     i = 0
    for i in tqdm(range(len(train_loader))):
        ...
        if i % opt.valInterval == 0:
            ...
            acc_tmp = val(test_dataset, criterion, max_iter=200)
            if acc_tmp > acc:
                acc = acc_tmp
